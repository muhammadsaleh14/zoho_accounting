// File: apps/web-plugin/src/pages/DashboardPage.tsx
...
export function DashboardPage() {
  // NOTE: We will use dummy data for now.
  // In a later step, we will connect this to your real backend API.
  const { data: invoices, isLoading } = useQuery<Invoice[]>({
    queryKey: ["invoices"],
    queryFn: api.getInvoices, // This will be updated later
  });
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 lg:mb-10 gap-4">
        <div>
          ...
          <p className="text-sm lg:text-base text-slate-500 mt-2 font-medium">
            Your financial ecosystem is looking healthy.{" "}
            <span className="text-brand-600">8 documents</span> need your
            attention.
          </p>
        </div>
        <div className="flex gap-2 w-full lg:w-auto">
          <button className="premium-button-secondary bg-white flex-1 lg:flex-none justify-center">
            View Statistics
          </button>
          <button className="premium-button-primary flex-1 lg:flex-none justify-center">
            Create New Report
          </button>
        </div>
      </div>
              ...
              <table className="w-full text-left border-collapse min-w-[600px]">
                <thead>
                  <tr className="bg-slate-50/50 border-b border-slate-100">
                    <th className="px-4 lg:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Document
                    </th>
                    <th className="px-4 lg:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      Status
                    ...
                    <th className="px-4 lg:px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">
                      Amount
                    </th>
                  </tr>
                </thead>
                    ...
                    ? Array(5)
                        ...
                        .map((_, i) => (
                          <tr key={i} className="animate-pulse">
                            <td
                              colSpan={4}
                              className="px-6 py-4 h-12 bg-slate-50/50"
                            ></td>
                          </tr>
                        ))
                    : recentDocs.map((doc) => (
                        ...
                        >
                          <tr className="hover:bg-slate-50/80 transition-all group cursor-pointer">
                            ...
                            <td className="px-4 lg:px-6 py-4 text-xs font-semibold text-slate-600">
                              {doc.date}
                            </td>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card p-6 border-l-4 border-l-emerald-500">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-4">
              Direct audit of your last 50 documents shows 98% TRN compliance.
          ...
          <div className="glass-card p-6 border-l-4 border-l-amber-500">
            ...
            <p className="text-xs text-slate-500 leading-relaxed mb-4">
              You have 8 documents waiting for internal bookkeeping review.
              Estimated completion:{" "}
              <span className="font-bold text-slate-700">24 hours</span>.
            </p>
