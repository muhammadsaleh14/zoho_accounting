    ...
    return (
        <div className="max-w-4xl mx-auto py-4">
            ...
            <div className="flex justify-between items-end mb-10">
                <div>
                    <h1 className="text-2xl lg:text-4xl font-black text-slate-900 tracking-tight">Notifications</h1>
                ...
                {notifications.map((notif) => {
                    ...
                    return (
                        <div
                            key={notif.id}
                            className={`
                glass-card p-4 lg:p-6 flex gap-4 lg:gap-6 transition-all group relative
                ${!notif.read ? "border-l-4 border-l-brand-600 bg-brand-50/10" : "hover:bg-slate-50/50"}
              `}
                        >
                            <div className={`${styles.bg} ${styles.color} p-3 lg:p-4 rounded-2xl h-fit shadow-sm shrink-0`}>
                                <styles.icon size={24} />
                            </div>

                            <div className="flex-1 min-w-0">
                                <div className="flex flex-col lg:flex-row justify-between items-start mb-1 gap-2 lg:gap-0">
                                    <h3 className={`text-base lg:text-lg font-bold leading-tight ${!notif.read ? "text-slate-900" : "text-slate-700"}`}>
                                        {notif.title}
                                    </h3>
                                ...
                                {!notif.read && (
                                    <button className="mt-3 lg:mt-4 text-[10px] font-black text-brand-600 uppercase tracking-widest hover:underline flex items-center gap-1">
