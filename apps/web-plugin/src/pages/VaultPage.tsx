...
export function VaultPage() {
    ...
    const outputReports = [
        { name: 'Monthly Profit & Loss', date: 'Dec 2025', size: '1.2 MB', type: 'PDF' },
        { name: 'Balance Sheet (Q4)', date: 'Oct-Dec 2025', size: '2.4 MB', type: 'PDF' },
        { name: 'Cash Flow Statement', date: 'Dec 2025', size: '890 KB', type: 'PDF' },
        { name: 'AP Aging Report', date: 'Dec 24, 2025', size: '1.1 MB', type: 'XLSX' },
    ];
    ...
    return (
        <div className="max-w-6xl mx-auto py-2 lg:py-4 pb-20">
            ...
            <div className="mb-6 lg:mb-10">
                <h1 className="text-2xl lg:text-4xl font-black text-slate-900 tracking-tight">Document Vault</h1>
                <p className="text-sm lg:text-base text-slate-500 mt-2 font-medium">Manage your financial history and download generated reports.</p>
            </div>
                  ...
                  `}
                                >
                                    <div className="flex items-center gap-3">
                                        <cat.icon size={18} className={category === cat.id ? "text-brand-600" : "text-slate-400 group-hover:text-slate-900"} />
                                        <span className="whitespace-nowrap">{cat.label}</span>
                                    </div>
                                    <ChevronRight size={14} className={`hidden lg:block ${category === cat.id ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`} />
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="glass-card p-4 lg:p-6">
                        ...
                        <button className="premium-button-secondary w-full text-xs py-2 scale-95">Advanced Filters</button>
                    </div>
                </div>
                                        ...
                                        {isLoading ? (
                                            Array(8).fill(0).map((_, i) => (
                                                <tr key={i} className="animate-pulse">
                                                    <td colSpan={4} className="px-6 py-6 h-12 bg-slate-50/50"></td>
                                                </tr>
                                            ))
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] text-slate-400 font-medium">#{doc.id} • {doc.currency} {doc.amount}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <span className={doc.status === 'review' ? 'status-badge-yellow' : 'status-badge-green'}>
                                                        {doc.status}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-xs font-semibold text-slate-600">{doc.date}</td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                                        <button className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-brand-600 hover:border-brand-300 shadow-sm transition-all" title="Download">
                                                            <Download size={14} />
                                                        </button>
                                                        <button className="p-2 bg-white border border-slate-200 rounded-lg text-slate-500 hover:text-emerald-600 hover:border-emerald-300 shadow-sm transition-all" title="Email">
                                                            <Mail size={14} />
                                                        </button>
                                                    </div>
                                                </td>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {filteredReports.length === 0 ? (
                                <div className="md:col-span-2 glass-card p-12 text-center">
                                    ...
                                    <p className="text-xs text-slate-400">Your search didn't match any generated reports.</p>
                                </div>
                            ) : filteredReports.map((report, i) => (
                                <div key={i} className="glass-card p-6 flex flex-col justify-between hover:scale-[1.02] transition-all group cursor-pointer border-l-4 border-l-emerald-500">
                                    <div className="flex justify-between items-start mb-4">
                                        <div className="bg-emerald-50 p-3 rounded-2xl text-emerald-600">
                                            <FileCheck size={24} />
                                        </div>
                                        <div className="flex gap-2">
                                            <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 transition-all"><Download size={16} /></button>
                                            <button className="p-2 hover:bg-slate-100 rounded-lg text-slate-400 transition-all"><Mail size={16} /></button>
                                        </div>
                                    </div>
                                    <div>
                                        <h4 className="text-sm font-bold text-slate-800 mb-1 group-hover:text-brand-600 transition-colors">{report.name}</h4>
                                        <p className="text-xs text-slate-500 font-medium flex items-center gap-2">
                                            {report.date} • {report.size} • <span className="text-emerald-600 font-bold tracking-widest">{report.type}</span>
                                        </p>
                                    </div>
                                </div>
                            ))}
